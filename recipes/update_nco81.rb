...
remote_file "#{node['objsrv']['install_dir']}/#{node['objsrv']['fp_pkg']}" do
  source "#{node['objsrv']['media_url']}/#{node['objsrv']['fp_pkg']}"
  not_if { File.exist?("#{node['objsrv']['install_dir']}/#{node['objsrv']['fp_pkg']}") }
...
execute 'unzip_fp_package' do
  command "unzip -q #{node['objsrv']['install_dir']}/#{node['objsrv']['fp_pkg']}"
...
file "#{node['objsrv']['install_dir']}/#{node['objsrv']['fp_pkg']}" do
